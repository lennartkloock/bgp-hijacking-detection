...
Das etablierte Protokoll, das diesen Austausch ermöglicht, nennt sich _Border Gateway Protocol_ (kurz: BGP).
Mithilfe der Informationen, die durch BGP-Nachrichten von den anderen AS empfangen werden, pflegt jeder Router eine eigene Routing-Tabelle, die den aktuellen Zustand des Netzwerks aus Sicht dieses Routers abbildet.
...
Eine OPEN-Nachricht wird einmalig beim Herstellen der Verbindung zwischen beiden Routern ausgetauscht.
...
Wenn mit einer UPDATE-Nachricht eine neue Route verkündet wird, enthält sie ein NEXT_HOP-Attribut, welches angibt an welche IP-Adresse ein Datenpaket weitergeschickt werden soll um von der verkündeten Route Gebrauch zu machen.
@rfc1654
